...
export class RecommendationService {
  ...
  static async getRecommendedInfluencers(campaignId: string, topN: number = 15): Promise<RecommendedInfluencer[]> {
    ...
    const scored = influencers.map(influencer => {
      ...
      if (profile.followerCount >= 10000) followerTierScore = 1;
      else if (profile.followerCount >= 5000) followerTierScore = 0.7;
      else if (profile.followerCount >= 1000) followerTierScore = 0.4;
