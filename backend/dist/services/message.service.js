...
class MessageService {
    // US-5.1: Send Message
    static async sendMessage(campaignId, senderId, receiverId, text) {
        ...
        const senderApplication = await index_1.prisma.campaignApplication.findFirst({
            where: {
                campaignId,
        ...
        if (senderIsBrand) {
            ...
            const receiverApplication = await index_1.prisma.campaignApplication.findFirst({
                where: {
                    campaignId,
        ...
        const sanitizedText = (0, xss_1.default)(text.trim(), {
    ...
    static async getMessageThread(campaignId, userId, otherUserId) {
        const messages = await index_1.prisma.message.findMany({
            where: {
                campaignId,
                OR: [
